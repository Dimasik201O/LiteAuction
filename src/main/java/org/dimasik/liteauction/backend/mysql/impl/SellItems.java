                ...
                try (PreparedStatement selectStatement = connection.prepareStatement(
                    ...
                    try (ResultSet rs = selectStatement.executeQuery()) {
                        while (rs.next()) {
                            ...
                            LiteAuction.getInstance().getRedisManager().publishMessage("msg", ItemHoverUtil.getHoverItemMessage(Parser.color("&#00D4FB▶ &#9AF5FB%item%&f &#9AF5FBx" + itemStack.getAmount() + " &fоказался слишком дорогой или никому не нужен. Заберите предмет с Аукциона!"), itemStack));
